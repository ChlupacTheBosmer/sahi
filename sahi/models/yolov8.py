...
class Yolov8DetectionModel(DetectionModel):
    ...
    def perform_inference(self, image: np.ndarray):
        ...
        """
        # Load config
        yolo_config = {} if self.config is None else self.config # Load the config attribute that is constructed from kwargs passed to DetectionModel constructor
        ...
        if self.image_size is not None:  # ADDED IMAGE SIZE OPTION FOR YOLOV8 MODELS:
            prediction_result = self.model(
                image[:, :, ::-1], imgsz=self.image_size, verbose=False, device=self.device, **yolo_config
            )  # YOLOv8 expects numpy arrays to have BGR
        else:
            prediction_result = self.model(
                image[:, :, ::-1], verbose=False, device=self.device, **yolo_config
            )  # YOLOv8 expects numpy arrays to have BGR
        prediction_result = [
